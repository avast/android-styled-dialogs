...
public class SimpleListDialogFragment extends BaseDialogFragment {
	...
	if (items != null && items.length > 0) {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		    OnListItemSelectedListener onListItemSelectedListener = getDialogListener();
    ...
    private OnListItemSelectedListener getDialogListener() {
	final Fragment targetFragment = getTargetFragment();
	if (targetFragment != null) {
	    if (targetFragment instanceof OnListItemSelectedListener) {
		return (OnListItemSelectedListener) targetFragment;
	    }
	} else {
	    if (getActivity() instanceof OnListItemSelectedListener) {
		return (OnListItemSelectedListener) getActivity();
	    }
	}
	return null;
    }
